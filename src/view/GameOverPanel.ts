class GameOverPanel extends egret.Sprite {
	...
	private playStarAni() {
		var gameover: egret.Bitmap = new egret.Bitmap();	//标题背景图
		...
		if (this._issuccess) {
			...
			egret.Tween.get(chengzi).to({ scaleX: 1, scaleY: 1 }, 700, egret.Ease.circIn);
			...
			shizi.scaleY = 1.5;
			this.addChild(shizi);
			egret.Tween.get(shizi).to({ scaleX: 1, scaleY: 1 }, 700, egret.Ease.circIn);
